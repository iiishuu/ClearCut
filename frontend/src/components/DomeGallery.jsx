...
export default function DomeGallery({
  ...
  const resetToDefaultPosition = useCallback(() => {
    stopAutoRotation();
    stopInertia(); // Arrêter l'inertie avant de commencer le reset
    const targetX = 0;
    // Ne pas réinitialiser Y, garder la position horizontale actuelle
    ...
    const animate = (currentTime) => {
      ...
      // Garder la position horizontale (y) actuelle sans la modifier

      rotationRef.current = { x: currentX, y: rotationRef.current.y };
      applyTransform(currentX, rotationRef.current.y);

      if (progress < 1) {
        requestAnimationFrame(animate);
      } else {
        // Une fois la réinitialisation terminée, reprendre l'auto-rotation
        startAutoRotation();
      }
    };
  ...
  const startAutoRotation = useCallback(() => {
    stopAutoRotation();
    const rotationSpeed = 0.15; // degrees per frame (~24 seconds per full rotation)
  ...
  }, [clearResetTimeout, resetToDefaultPosition]);
    ...
    onDrag: ({ event, last, velocity: velArr = [0, 0], direction: dirArr = [0, 0], movement }) => {
      ...
      if (last) {
        ...
        if (!isTap && (Math.abs(vx) > 0.005 || Math.abs(vy) > 0.005)) {
          startInertia(vx, vy);
          // Planifier le reset immédiatement après le relâchement, même avec inertie
          scheduleReset();
        } else if (!isTap) {
          // Drag ended without enough velocity for inertia, schedule reset
          scheduleReset();
        }
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      if (!originalPos) {
        ...
        // Reprendre l'auto-rotation après fermeture de l'image
        scheduleReset();
        return;
      }
  ...
  }, [enlargeTransitionMs, openedImageBorderRadius, grayscale, scheduleReset]);
  ...
  }, [startAutoRotation, stopAutoRotation, clearResetTimeout]);
